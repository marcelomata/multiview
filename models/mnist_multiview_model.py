...
class MnistMultiviewModel(BaseModel):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('out'):
            self.final_W = tf.Variable(tf.truncated_normal([self.config.num_hidden_neurons,self.config.num_classes], stddev=0.01))
            self.final_b = tf.Variable(tf.zeros(self.config.num_classes))
            self.out = tf.matmul(self.z, self.final_W)+self.final_b
        ...
        with tf.variable_scope('loss'):
            ...
            with tf.variable_scope('loss_acc'):
                ...
                #self.loss_node = (kl+x_log_lik+y_log_lik)/self.config.batch_size
    ...
    def softmax_sampler(self, z_mean, z_stddev, num_samples, batch_size, num_hidden_neurons, name='test'):
        with tf.variable_scope(name):
            ...
            print(tf.nn.softmax(tf.einsum('aij,jk->aik', zs, self.final_W)))
            print(tf.expand_dims(self.final_b, 1))
            softmaxs = tf.nn.softmax(tf.einsum('aij,jk->aik', zs, self.final_W)+self.final_b)
